...
import { installMockCheckinApi } from "./mock-checkin-api";
test.beforeEach(async ({ page }) => {
  await installMockCheckinApi(page);
});
...
test.describe("guest check-in", () => {
  ...
  for (const [token, prompt] of roundCases) {
    test(`${token} loads its dedicated scenario`, async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "정기 체크인" }),
      ).toBeVisible();
  ...
  test("collects at most two distinct monthly issues", async ({ page }) => {
    ...
    await page
      .getByRole("textbox", { name: "추가로 전할 내용" })
      .fill("천장에서 물이 떨어져요");
    await page.getByRole("button", { name: "보내기", exact: true }).click();
    await page.getByRole("button", { name: "네, 더 있어요" }).click();

    await expect(page.getByRole("button", { name: "시설·수리" })).toHaveCount(
      0,
    );
    ...
    await expect(
      page.getByRole("textbox", { name: "추가로 전할 내용" }),
    ).toBeVisible();
    await expect(
      page.getByRole("textbox", { name: "추가로 전할 내용" }),
    ).toHaveValue("");
    await expect(
      page.getByRole("button", { name: "네, 더 있어요" }),
    ).toHaveCount(0);
    await page.getByRole("button", { name: "건너뛰기" }).click();
    await expect(
      page.getByText("응답이 저장됐어요", { exact: false }),
    ).toBeVisible();
  });

  test("collects urgent detail and text without the additional issue loop", async ({
    page,
  }) => {
    await page.goto("/c/demo-monthly");
    ...
    await expect(
      page.getByText("바로 확인해서", { exact: false }),
    ).toBeVisible();
    await expect(
      page.getByText("응답이 저장됐어요", { exact: false }),
    ).toBeVisible();
...
test("positive monthly answers end in the independent community interest flow", async ({
  page,
}) => {
  const events: { type: string; clicked: boolean; topics: string[] }[] = [];
  page.on("response", async (response) => {
    if (response.url().endsWith("/interest-events") && response.ok()) {
      events.push({
        ...(await response.json()),
        type: response.request().postDataJSON().type,
      });
    }
  });
  await page.goto("/c/demo-monthly");
  await expect(
    page.getByRole("button", { name: "궁금해요, 알려주세요" }),
  ).toHaveCount(0);
  await page.getByRole("button", { name: "네, 다 괜찮아요" }).click();
  await expect(
    page.getByText(
      "다행이에요! 혹시 아주 사소하게라도 신경 쓰였던 건 없으셨어요?",
    ),
  ).toBeVisible();
  ...
  const teaser = page.getByRole("region", {
    name: "나만 이런가? 다른 분들은 어떻게 지내는지 궁금하셨죠?",
  });
  await teaser.scrollIntoViewIfNeeded();
  await expect
    .poll(() => events.filter((event) => event.type === "exposed").length)
    .toBe(1);
  ...
  await expect(
    teaser.getByRole("button", { name: "주방·요리" }),
  ).toHaveAttribute("aria-pressed", "true");
  await teaser.getByRole("button", { name: "선택 완료" }).click();
  await expect(
    teaser.getByText("고마워요! 준비되면 알려드릴게요 🙂"),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByRole("log")
      .getByText("다음 달에 또 가볍게 여쭤볼게요", { exact: false }),
  ).toHaveCount(1);
});

test("already completed sessions can submit interest with no topic selected", async ({
  page,
}) => {
  await page.goto("/c/demo-completed");
  await expect(
    page.getByRole("heading", { name: "이번 체크인은 이미 답변해 주셨어요" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "궁금해요, 알려주세요" }).click();
  await page.getByRole("button", { name: "선택 완료" }).click();
  await expect(
    page.getByText("고마워요! 준비되면 알려드릴게요 🙂"),
  ).toBeVisible();
});

test("every supported guest round completes through text and offers community interest", async ({
  page,
}) => {
  const cases = [
    {
      token: "demo-onboarding",
      label: "네, 잘 적응하고 있어요",
      regular: true,
    },
  ...
  for (const entry of cases) {
    ...
    if (entry.token === "demo-renewal")
      await page.getByRole("button", { name: "계속 살고 싶어요" }).click();
...
for (const width of [320, 390]) {
  test(`mobile ${width}px renders local fonts, logo, urgent text and interest without overflow`, async ({
    page,
  }, testInfo) => {
    ...
    await page
      .getByRole("button", { name: "그 외 (직접 입력)", exact: true })
      .click();
    await expect(page.getByRole("textbox")).toBeVisible();
    await page
      .getByRole("textbox")
      .fill("상황 설명입니다. 담당 매니저의 도움이 필요해요.");
    ...
    await page.screenshot({
      path: testInfo.outputPath("community-topics.png"),
    });
    const overflows = await page.locator("main").evaluate((main) => {
      const width = document.documentElement.clientWidth;
      return [...main.querySelectorAll("button, textarea, h1, h2")]
        .filter((element) => {
