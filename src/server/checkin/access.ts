...
export function signGrant(grant: Grant) {
  ...
  return (
    encoded +
    "." +
    createHmac("sha256", accessEnvironment().secret)
      .update(encoded)
      .digest("base64url")
  );
}
export function verifyGrant(value: string): Grant {
  try {
    ...
    if (rest.length || !encoded || !signature || value.length > 1024)
      throw new Error();
    const actual = Buffer.from(signature, "base64url");
    const expected = createHmac("sha256", accessEnvironment().secret)
      .update(encoded)
      .digest();
    ...
    if (!Number.isFinite(grant.exp) || grant.exp <= Date.now())
      throw new CheckinError("expired", 410);
    return grant;
  } catch (e) {
    if (e instanceof CheckinError && e.code === "expired") throw e;
...
export async function exchangeToken(token: unknown) {
  ...
  const { data, error } = await db
    ...
    .eq("token_digest", digestToken(token))
    .maybeSingle();
  if (error) databaseError(error);
  if (!data) throw new CheckinError("invalid", 401);
  const loaded = await load(data.id, data.session_id);
  if (
    loaded.row.status !== "completed" &&
    Date.parse(loaded.row.answer_expires_at) <= Date.now()
  )
    throw new CheckinError("expired", 410);
  const grant = {
    tokenId: data.id,
    sessionId: data.session_id,
    exp: Date.parse(loaded.token.expires_at),
  };
