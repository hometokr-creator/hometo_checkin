...
import type {
  ...
  CheckinIssue,
  CheckinIssueTag,
} from "@/domains/checkin/model/checkin";
...
const level = (tag: CheckinIssueTag) =>
  tag === "urgent" ? ("R1" as const) : ("R2" as const);
function withoutTriage(issues: CheckinIssue[]) {
  return issues.map((i) => ({
    tag: i.tag,
    detail: i.detail,
    freeText: i.freeText,
...
export function validateSubmission(input: unknown, row: SessionRow) {
  const submission = object(input);
  keys(submission, ["schemaVersion", "sessionId", "idempotencyKey", "answers"]);
  ...
  if (
    ...
    submission.idempotencyKey.length > 200
  )
    reject();
  const raw = object(submission.answers);
  keys(raw, ["responses", "issues", "freeText", "overallTriage"]);
  ...
  const given: CheckinIssue[] = raw.issues.map((value) => {
    ...
    keys(i, ["tag", "detail", "freeText", "triageLevel"]);
    if (
      typeof i.tag !== "string" ||
      !tags.includes(i.tag) ||
      (i.detail !== undefined &&
        (typeof i.detail !== "string" || i.detail.length > 80))
    )
      reject();
    if (
      i.triageLevel !== undefined &&
      i.triageLevel !== "R1" &&
      i.triageLevel !== "R2"
    )
      reject();
    return {
      tag: i.tag as CheckinIssueTag,
      detail: i.detail as string | undefined,
      freeText: text(i.freeText),
      triageLevel: level(i.tag as CheckinIssueTag),
    };
  });
  if (new Set(given.map((i) => i.tag)).size !== given.length) reject();
  if (
    raw.overallTriage !== undefined &&
    raw.overallTriage !== "R1" &&
    raw.overallTriage !== "R2"
  )
    reject();
  ...
  function advance(next: ScenarioNext): void {
    ...
    if (next.type === "issue-count") {
      stepId =
        expected.issues.length < next.lessThan ? next.then : next.otherwise;
      return;
    }
    if (next.type === "complete-from-answers") {
      expected.issues = expected.issues.filter(
        (i) => i.tag !== "other" || !!(i.freeText || expected.freeText),
      );
      outcome = expected.issues.some((i) => i.tag === "urgent")
        ? "urgent"
        : expected.issues.length || expected.freeText
          ? "reported"
          : "ok";
    } else {
      if (next.outcome === "renewal") reject();
  ...
  for (let count = 0; stepId && count < 32; count++) {
    ...
    if (control.kind === "options") {
      ...
      if (!option) reject();
      if (option.presetIssueTag)
        expected.issues.push({
          tag: option.presetIssueTag,
          triageLevel: level(option.presetIssueTag),
        });
      advance(option.next);
    } else if (control.kind === "tags") {
      const tag = control.tags.find((t) => t.value === value);
      if (
        !tag ||
        expected.issues.length >= 2 ||
        expected.issues.some((i) => i.tag === tag.value)
      )
        reject();
      expected.issues.push({ tag: tag.value, triageLevel: level(tag.value) });
      advance(control.nextByTag[tag.value]);
    } else if (control.kind === "chips") {
      const issue = expected.issues.at(-1);
      if (
        !issue ||
        !getCheckinDetailOptions(issue.tag).some((d) => d.value === value)
      )
        reject();
      issue.detail = value;
      ...
      const issue = expected.issues.at(-1);
      const supplied =
        control.target === "issue"
          ? given.find((i) => i.tag === issue?.tag)?.freeText
          : globalText;
      if ((value === "provided") !== !!supplied) reject();
      if (control.target === "issue") {
        if (!issue) reject();
        if (supplied) issue.freeText = supplied;
      } else if (supplied) {
  ...
  if (
    stepId ||
    !outcome ||
    canonical(responses) !== canonical(expected.responses) ||
    canonical(withoutTriage(given)) !==
      canonical(withoutTriage(expected.issues)) ||
    globalText !== expected.freeText
  )
    reject();
  expected = {
    ...expected,
    overallTriage: expected.issues.some((i) => i.tag === "urgent")
      ? "R1"
  ...
  const requestHash = createHash("sha256")
    .update(
      canonical({
        ...
        triageRuleVersion: row.triage_rule_version,
        answers: expected,
      }),
    )
    .digest("hex");
  const positiveFeedback =
    outcome === "reported" &&
    !!expected.freeText &&
    (expected.responses.facilityEventStatus === "resolved" ||
      expected.responses.ruleEventStatus === "understood");
  return {
    answers: expected,
    outcome,
    requestHash,
    idempotencyKey: submission.idempotencyKey,
    completionMessage: positiveFeedback
      ? "의견 남겨주셔서 감사해요! 잘 전달할게요 🙂"
      : scenario.completionMessages[outcome].text,
  };
}
