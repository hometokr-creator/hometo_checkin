...
export async function POST(request: Request) {
 try {
  const input=await body(request,2048);keys(input,["token"]);
  const {grant,value}=await exchangeToken(input.token);
  const response=json({sessionId:grant.sessionId});
  response.cookies.set(cookieName(grant.sessionId),value,{httpOnly:true,secure:accessEnvironment().secure,sameSite:"lax",path:"/api/checkin",expires:new Date(grant.exp)});
  return response;
